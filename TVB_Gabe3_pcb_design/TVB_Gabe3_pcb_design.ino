...
//////////GABRIEL STAPLES'S EDITS: SEE "main.h" TO VERIFY PINS TO USE//////////////
Circuit:
...
//GS: instantiate an object of this library class; call it "button1"
...
//eRCaGuy_ButtonReader button1 = eRCaGuy_ButtonReader(TRIGGER); 
//alternatively, the object instantiation could be explicit, as follows:
eRCaGuy_ButtonReader button1 = eRCaGuy_ButtonReader(TRIGGER,50,BUTTON_PRESSED);
...
//  digitalWrite(EXTRA_GND_PIN,LOW); //make this pin an extra GND pin, for the pushbutton or for the REGIONSWITCH pin or whatever else you need it for
  ...
  digitalWrite(TRIGGER, HIGH); //pull-up
...
//  }
  ...
  //if the user pushes the Trigger button, then start transmission of all POWER codes
  if (button_action==1) //true, debounced button PRESS detected
  {
    //wait for button RELEASE
    while (button_action!=-1)
    {
      button1.readButton(&button_action,&button_state); //read button
    }
    sendAllCodes(); //once button is RELEASED, send all codes
  }
} //end of loop()
...
//  //GS edit:
//  detachInterrupt(1);                         // disables int 1 as the wakeupnow code will
                                              // not be executed during normal runtime
}
